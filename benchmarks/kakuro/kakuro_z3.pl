...
kakuro(ID) :-
	...
	write('Z3\n'),
	format('Id, execution time, labeling time\n', []),
	format('~w, ~3d, ~3d\n', [ID, T1-T0, P1-P0]),
	z3_stop.
...
all_diff_sum(_, L, Sum) :-
	...
	Series is N*(N+1)//2,
	(   Sum=:=Series ->
	    to_smtlib([domain(L, 1, N)])
	;   Sum=:=10*N-Series ->
	    Min is 10-N,
	    to_smtlib([domain(L, Min, 9)])
	), !,
	to_smtlib([all_distinct(L)]).
all_diff_sum(global, L, Sum) :-
	plusify(L, Expr),
	to_smtlib([all_distinct(L, [Expr #= Sum])]).
all_diff_sum(gcc, L, Sum) :-
	same_length(L, Matrix),
	(   foreach([1,2,3,4,5,6,7,8,9],Matrix)
	do  true
	),
	to_smtlib([domain([B1,B2,B3,B4,B5,B6,B7,B8,B9], 0, 1)]),
	to_smtlib([global_cardinality(L, [1-B1,2-B2,3-B3,4-B4,5-B5,6-B6,7-B7,8-B8,9-B9], [cost(Sum,Matrix)])]).
